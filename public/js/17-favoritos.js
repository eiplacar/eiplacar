// ═══════════════════════════════════════════════════
// FAVORITOS (Favorita Ponto) — motor de cálculo usado pela página Favoritos, no menu
// lateral (src/components/Favoritos.jsx). Antes vivia como uma 3ª sub-aba "Índice" dentro
// de Análise; essa sub-aba foi removida e a análise (+ a lista de favoritados) virou página
// própria — este arquivo só mudou de nome (17-indice.js → 17-favoritos.js), a lógica é a
// mesma. Não recalcula nada do zero: pega o resultado que computeAnalise() (09-analise.js)
// já calculou e cruza os indicadores com peso, gerando 3 pontuações independentes —
// Resultado, Gols e BTTS — em vez de misturar tudo numa nota só.
//
...
// Classificação única (5 níveis) usada em TODO indicador do app — Resultado, Gols (cada
// linha) e BTTS. Antes Resultado/BTTS usavam uma escala de 6 níveis e as linhas de Gols
// usavam outra de 5, com nomes e cores diferentes entre si; agora é uma coisa só, com as
// cores oficiais do EI PLACAR:
//   Forte #168A45 · Favorável #35A853 · Moderado #F2C94C · Arriscado #F2994A · Muito arriscado #D64545
function classificar(pontuacao){
  if(pontuacao>=90) return 'Forte';
  if(pontuacao>=70) return 'Favorável';
  if(pontuacao>=50) return 'Moderado';
  if(pontuacao>=25) return 'Arriscado';
...
function computeIndiceGols(data){
  ...
  return { pontuacao, classificacao: classificar(pontuacao), linhas };
}
...
async function favoritarIndice(data, idx, jogo){
  ...
  const payload = {
    ...
    gols_linha1: idx.gols?.linhas?.[0]?.linha||null,
    gols_prob1: idx.gols?.linhas?.[0]?.pontuacao??null,
    gols_linha2: idx.gols?.linhas?.[1]?.linha||null,
    gols_prob2: idx.gols?.linhas?.[1]?.pontuacao??null,
    gols_linha3: idx.gols?.linhas?.[2]?.linha||null,
    gols_prob3: idx.gols?.linhas?.[2]?.pontuacao??null,
    gols_linha4: idx.gols?.linhas?.[3]?.linha||null,
    gols_prob4: idx.gols?.linhas?.[3]?.pontuacao??null,
