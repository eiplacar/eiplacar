...
// some sozinho da lista 120 minutos depois do horário marcado.
// ═══════════════════════════════════════════════════
...
function ophExpirado(j){
  ...
  const limite = new Date(base.getTime() + 120*60*1000); // some da lista 120min depois do início do jogo
...
function ophRenderLista(){
  let lista = ophCache.filter(j=>!ophExpirado(j)); // some sozinho 120min depois do horário (só da TELA — quem apaga de verdade é o organizador)
  ...
  const html = lista.map(j=>{
    ...
    // em vermelho, ex: "Começa em 2h30"). Jogo expirado (120min depois do início) já
    ...
    <div onclick="ophToggleSelecao(${j.id})" style="flex:0 0 auto;width:150px;background:var(--c2);border:2px solid ${sel?'var(--verde2)':'var(--c3)'};border-radius:10px;padding:24px 10px 10px;text-align:center;position:relative;cursor:pointer">
      ...
      <div style="font-size:11.5px;font-weight:700;line-height:1.2;margin-top:8px">${j.casa||'—'}</div>
      <div style="height:8px"></div>
      <div style="width:30px;height:30px;margin:0 auto">${escudoImgOuIcone(j.vis)}</div>
      <div style="font-size:11.5px;font-weight:700;line-height:1.2;margin-top:8px">${j.vis||'—'}</div>
